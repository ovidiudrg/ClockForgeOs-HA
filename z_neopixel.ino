  ...
  extern bool displayON; // day/night runtime state from ClockForgeOS.ino
  ...
  // Default for day mode auto-dimming floor (percent of rgbBrightness at very low lux).
  // Runtime value comes from settings.rgbAutoDimDayMinPercent (UI configurable).
  #ifndef RGB_AUTODIM_DAY_MIN_PERCENT
    #define RGB_AUTODIM_DAY_MIN_PERCENT 25
  #endif
  ...
  // Drive exactly the configured tube LED mapping.
  // Padding "virtual" pixels can cause partial/truncated updates on busy ESP32 loops,
  // which looks like only the first LEDs reacting while tail LEDs stay stale.
  const int PixelCount = TubePixelMapCount;
  const int StripPixelCount = TubePixelMapCount;

  inline bool isMappedActivePixel(int idx) {
    #if defined(CLOCK_42) || defined(CLOCK_42a)
    // CLOCK_42x uses a fixed 6-pixel mapping (0..5). Do not depend on runtime maxDigits here.
    // If maxDigits is ever transient/corrupted, RGB would appear "frozen" except one/few LEDs.
    return (idx >= 0) && (idx < TubePixelMapCount) && (tubePixels[idx] < TubePixelMapCount);
    #else
  ...
  //NeoBrgFeature give me BGRW (g and r swapped)
    ...
    #if defined(CLOCK_42) || defined(CLOCK_42a)
      // CLOCK_42/42a: use a non-default RMT channel to avoid runtime contention/freezes.
      NeoPixelBrightnessBus<NeoGrbFeature, NeoEsp32Rmt1Ws2812xMethod> strip(StripPixelCount,PixelPin);
    #else
      NeoPixelBrightnessBus<NeoGrbFeature, NeoEsp32Rmt0Ws2812xMethod> strip(StripPixelCount,PixelPin);
    #endif
  #else
  ...
  #if defined(ESP32) && (defined(CLOCK_42) || defined(CLOCK_42a))
    // Single show is more stable on this profile when multiplex + WiFi + MQTT are active.
    static const bool NEO_DOUBLE_SHOW = false;
  #else
    static const bool NEO_DOUBLE_SHOW = true;
  #endif

  static inline void showStableFrame(bool forceDoubleFrame = false) {
    neoShowInProgress = true;
    strip.Show();
    #if defined(ESP32)
  ...
  void setupNeopixel() {
      ...
      DPRINT("Tube map count/maxDigits: "); DPRINT(TubePixelMapCount); DPRINT("/"); DPRINTLN(maxDigits);
  ...
  RgbColor Wheel(int WheelPos) {
      // Keep an explicit white sentinel outside normal 0..255 wheel range.
      ...
      WheelPos &= 255; // Ensure the position is within 0..255
  ...
  void showSolidNeopixels(uint8_t r, uint8_t g, uint8_t b, uint8_t brightness) {
    ...
    applyCurrentLimitAndShow((uint8_t)neoBrightness);
  }
  ...
  void rainbow2() {
      ...
      int colorIndex = j & 255;
      if (colorIndex == WHITE_INDEX) colorIndex = (WHITE_INDEX + 1) & 255;
      RgbColor color = Wheel((byte)colorIndex);
      setPixels(i, color);
  ...
  void effect3(boolean enableRandom,boolean eachPixelRandom) {
    ...
    if (actColor[i] != newColor[i]) {   //next rainbow color
      ...
      // WHITE_INDEX is reserved for alarm/explicit white. Never pass through it in animations.
      if (actColor[i] == WHITE_INDEX) {
        if (dir == 1) actColor[i] = min(newColor[i], WHITE_INDEX + 1);
        else actColor[i] = max(newColor[i], WHITE_INDEX - 1);
      }
    }

    for (int j=0;j<maxDigits;j++) {
      int renderIdx = actColor[j];
      if (renderIdx == WHITE_INDEX) {
        int d = (newColor[j] >= oldColor[j]) ? 1 : -1;
        renderIdx = constrain(renderIdx + d, 0, 255);
      }
      setPixels(j, Wheel((byte)renderIdx));
    }
  ...
  void effect4() {    //every pixel is random changer
    ...
    if (newColor[t] != actColor[t]) {   //next rainbow color
      ...
      if (actColor[t] == WHITE_INDEX) {
        if (oldColor[t] < newColor[t]) actColor[t] = min(newColor[t], WHITE_INDEX + 1);
        else actColor[t] = max(newColor[t], WHITE_INDEX - 1);
      }
    }
  ...
  bool forceImmediate = false;
    ...
    if (lastEffect != (int)prm.rgbEffect) {
      lastEffect = (int)prm.rgbEffect;
      neoPaused = false;
      forceImmediate = true;
      DPRINTF("[NEO] effect=%d rgbBrightness=%d maxLedmA=%u tubes=%d\n",
              lastEffect, (int)prm.rgbBrightness, (unsigned)prm.maxLedmA, (int)tubesPowerState);
    }
    ...
    // Runtime gate:
    // - RGB OFF if effect is disabled
    // - RGB OFF if tubes are OFF
    // - RGB OFF during automatic night mode when "RGB On/Off (Night Mode)" is OFF
    const bool isNightModeBlocked = prm.enableAutoShutoff && !displayON && (settings.rgbNightEnabled == 0);
    if ((prm.rgbEffect == 0) || !tubesPowerState || isNightModeBlocked) {   //switch RGB backlight OFF
      if (!neoPaused) {
        darkenNeopixels();
        neoPaused = true;
      }
      ...
      neoBrightness = prm.rgbBrightness;
      // If an effect is selected, avoid a fully dark runtime state from stale 0 brightness.
      if ((prm.rgbEffect > 0) && (neoBrightness <= 0)) {
        neoBrightness = c_MinBrightness;
      }
      if (autoBrightness) {
        neoBrightness = (neoBrightness * lx) / long(MAXIMUM_LUX);
        long minAutoDimBrightness = c_MinBrightness;
        if (displayON) { // apply day-only floor
          uint8_t dayMinPercent = settings.rgbAutoDimDayMinPercent;
          if (dayMinPercent > 100) dayMinPercent = RGB_AUTODIM_DAY_MIN_PERCENT;
          long dayFloor = ((long)prm.rgbBrightness * (long)dayMinPercent + 99L) / 100L;
          if (dayFloor > minAutoDimBrightness) minAutoDimBrightness = dayFloor;
        }
        if (neoBrightness < minAutoDimBrightness) {
          neoBrightness = minAutoDimBrightness;
        }
      }
    ...
    #if defined(CLOCK_42) || defined(CLOCK_42a)
    // On CLOCK_42x, effects 6/10 are often perceived as "frozen" (stepwise/per-pixel behavior).
    // Remap both to a smooth full-strip transition.
    else if (prm.rgbEffect==6) effect2();
    else if (prm.rgbEffect==10) effect2();
    #else
    ...
    else if (prm.rgbEffect==9) effect4();  //fireworks
    #if !defined(CLOCK_42) && !defined(CLOCK_42a)
    ...
    static unsigned long lastNeoDbgMs = 0;
    if ((millis() - lastNeoDbgMs) >= 2000UL) {
      lastNeoDbgMs = millis();
      RgbColor p0 = strip.GetPixelColor(0);
      int activePx = 0;
      for (int i = 0; i < PixelCount; i++) {
        if (isMappedActivePixel(i)) activePx++;
      }
      DPRINTF("[NEO] run e=%d bReq=%ld bAp=%u mA=%u maxDigits=%d px=%d/%d p0=%u,%u,%u\n",
              (int)prm.rgbEffect,
              neoBrightness,
              (unsigned)neoAppliedBrightness,
              (unsigned)neoAppliedCurrentmA,
              (int)maxDigits,
              activePx,
              PixelCount,
              (unsigned)p0.R, (unsigned)p0.G, (unsigned)p0.B);
    }
  }

  #else
  static inline void clampNeoDataPinLow() {
    #if defined(ESP32) && (defined(CLOCK_42) || defined(CLOCK_42a))
      #if defined(ESP32)
  ...
  void setupNeopixel() {
    #if defined(ESP32) && (defined(CLOCK_42) || defined(CLOCK_42a))
      // NeoPixel disabled: keep data line hard-low.
      clampNeoDataPinLow();
      DPRINTLN("[NEO] disabled: data pin forced LOW");
    #endif
  }
  void doAnimationMakuna() {
    #if defined(ESP32) && (defined(CLOCK_42) || defined(CLOCK_42a))
      // Re-assert low continuously in case other code touches this GPIO.
      clampNeoDataPinLow();
      static unsigned long lastNeoDisDbgMs = 0;
      if ((millis() - lastNeoDisDbgMs) >= 5000UL) {
        lastNeoDisDbgMs = millis();
        DPRINTF("[NEO] disabled runtime pin25=%d\n", (int)digitalRead(25));
      }
    #endif
  }
  void darkenNeopixels() {}
