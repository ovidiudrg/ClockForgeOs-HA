#line 1 "c:\\Users\\Dellus\\Documents\\Arduino\\ClockForgeOS\\README.md"
# ClockForgeOS
...
### Authentication + Security (Important for Mobile App)

#### `POST /auth/login`
- Content type: form-urlencoded
- Required field:
  - `password`
- Success: `200 application/json` with:
  - `ok: true`
  - `token`
  - `ttlSec`

Security behavior:
- Login attempts are rate-limited (`429 {"error":"too_many_requests"}`).
- Repeated bad credentials trigger lockout (`429 {"error":"login_locked","retrySec":...}`).
- If firmware is still using fallback password `ChangeMeNow!`, login is denied (`503 {"error":"default_password_not_allowed"}`).
- Invalid password returns `401 {"error":"invalid_credentials"}`.

Token usage:
- Send token as `X-Auth-Token` header (recommended).
- POST body fallback `authToken` is supported.
- URL query token fallback is intentionally not accepted anymore.

Set a strong password in your profile:
- In `clocks.h`, define `WEB_ADMIN_PASSWORD` to a unique secret before exposing the device beyond a trusted LAN.
...
MQTT field limits and format:
- `mqttBrokerAddr`: max 29 chars, supports `host` or `host:port` (default port `1883`).
- `mqttBrokerUser`: max 19 chars.
- `mqttBrokerPsw`: max 19 chars.
- MQTT discovery/data prefix default: `clockforgeos` (unless overridden in `clocks.h` via `#define MQTT_PREFIX "..."`).
...
Examples:
```bash
curl -X POST http://CLOCK_IP/setManualTime -H "X-Auth-Token: YOUR_TOKEN" -d "epoch=1765812000"
curl -X POST http://CLOCK_IP/setManualTime -H "X-Auth-Token: YOUR_TOKEN" -d "date=2026-02-13" -d "time=17:45"
```
...
Examples:
```bash
curl -H "X-Auth-Token: YOUR_TOKEN" "http://CLOCK_IP/connectWifi?ssid=MySSID&psw=MyPass"
curl -X POST http://CLOCK_IP/connectWifi -H "X-Auth-Token: YOUR_TOKEN" -d "ssid=MySSID" -d "psw=MyPass" -d "save=true"
curl -H "X-Auth-Token: YOUR_TOKEN" http://CLOCK_IP/wifiStatus
```
...
Examples:
```bash
curl -X POST http://CLOCK_IP/reset -H "X-Auth-Token: YOUR_TOKEN"
curl -X POST http://CLOCK_IP/factoryreset -H "X-Auth-Token: YOUR_TOKEN"
curl -X POST http://CLOCK_IP/firmwareupdate -H "X-Auth-Token: YOUR_TOKEN"
curl -X POST http://CLOCK_IP/cathodeProtect -H "X-Auth-Token: YOUR_TOKEN"
```
...
### 31) Web Auth Hardening for API + Mobile Clients
- Added login hardening for `POST /auth/login`:
  - minimum interval/rate limit between attempts
  - failed-attempt lockout window with `retrySec`
  - default fallback password blocked for login
- Updated token handling:
  - primary auth via `X-Auth-Token` header
  - POST body `authToken` fallback remains
  - URL query token fallback removed
- Added constant-time password comparison helper to reduce timing side-channel risk.
...
## MQTT Troubleshooting (HA Mosquitto)

If MQTT does not connect, check broker logs first:

- `Client <unknown> disconnected due to protocol error`
  - Usually indicates outdated firmware/web assets mismatch or invalid MQTT connect packet handling.
  - Update firmware + web UI files together.

- `Sending CONNACK ... (0, 5)` + `not authorised`
  - MQTT packet is valid, but credentials are rejected by broker.
  - Re-enter MQTT user/password in ClockForgeOS web UI and save.
  - Confirm credentials in broker exactly match.
  - Keep user/password within firmware limit (19 chars each).

Runtime behavior in current firmware:
- MQTT config changes (`mqttBrokerAddr`, `mqttBrokerUser`, `mqttBrokerPsw`, `mqttEnable`) are saved immediately.
- MQTT client is reconfigured immediately after saving those keys (no full device reboot required).
